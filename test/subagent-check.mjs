...
import { collectSubagents } from '../src/index.mjs'
...
await t('只统计直接子会话（无关会话不计入）', async () => {
  const r = await collectSubagents(makeCtx(), PARENT, SETTINGS)
...
await t('子代理模型未定价 → 合计金额为 null（不给半真的数）', async () => {
  const r = await collectSubagents(makeCtx({ childModel: 'no-such-model' }), PARENT, SETTINGS)
...
await t('枚举失败 → 带 unavailable，不崩、不假装没有子代理', async () => {
  const r = await collectSubagents(makeCtx({ throwList: true }), PARENT, SETTINGS)
  assert.equal(r.count, 0)
  assert.match(r.unavailable, /query down/)
})
await t('没有 parentId → 空结果', async () => {
  const r = await collectSubagents(makeCtx(), null, SETTINGS)
  assert.equal(r.count, 0)
})
