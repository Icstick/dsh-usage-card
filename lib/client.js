		  ...
		  return h('div', { style: box },
		    h('div', { style: rowStyle },
		      ...
		      h('span', { style: { display: 'flex', gap: '8px', alignItems: 'baseline' } },
		        ...
		        showAmount && c.pricing && c.pricing.mode !== 'per-turn'
		          ? h('span', {
		              title: '该会话在插件加载前已产生的用量没有逐轮时间戳，按当前档位线性估算；加载后的部分按每轮实际时刻定价。覆盖 ' + Math.round((c.pricing.coverage || 0) * 100) + '%',
		              style: { ...muted, border: '1px solid ' + HAIRLINE, borderRadius: '4px', padding: '0 4px', fontSize: '10px' },
		            }, '含估算')
		          : null)),
