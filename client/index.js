...
function Ring(props) {
  const { data } = props
  // 与宽态**同一道形状闸**：ok 为真但 measured/cost 缺失时不能直接取属性 ——
  // 否则渲染期抛错（边界能兜住，但整块卡片会退化成"渲染失败"）。宁可显示成空环。
  const ok = payloadSane(data)
  const rate = ok ? data.measured.cacheHitRate : null
  const p = rate == null ? 0 : Math.max(0, Math.min(1, rate))
  const len = 2 * Math.PI * 13.5
  const cny = ok && Number.isFinite(data.cost.totalCny) ? data.cost.totalCny : null
  const title = ok
    ? '本会话 ' + (cny == null ? '—' : fmtCny(cny)) + ' · ' + fmtCount(data.measured.totalTokens) + ' tok · 命中 ' + fmtPct(rate)
    : '用量卡片：暂无数据'
  const money = ok && cny != null && (!data.display || data.display.showAmount !== false) ? fmtCny(cny) : null
  return h('div', {
    title,
...
function Card(props) {
  ...
  return h('div', { style: box },
    ...
    (attr == null || !showAttr || !detailOpen) ? null : h('div', null,
      ...
      // 收口：上面六行的百分比是 **token 占比**（描述上下文＝输入侧），金额只摊输入侧；
      // 输出侧单独计价。不把这两行摆出来，就会显得「六行加起来 ≠ 头部金额」。
      showAmount && c.outputUsd != null
        ? h('div', { style: { borderTop: '1px solid ' + HAIRLINE, marginTop: '6px', paddingTop: '5px' } },
            ...
            h('div', { style: rowStyle },
              ...
              h('span', { style: { ...mono, marginTop: '2px', fontWeight: 600, minWidth: '48px', textAlign: 'right' } }, fmtCny(c.totalCny))))
        : null,
