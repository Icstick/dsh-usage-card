...
function makeSettingsSection(scope) {
  return function UsageCardSettings() {
    ...
    useEffect(() => {
      let alive = true
      fetch('/usage-card/sessions', { cache: 'no-store' })
        .then((r) => r.json())
        .then((d) => {
          if (!alive) return
    ...
    return h('div', { style: { padding: '2px' } },
      ...
      h('p', { style: { ...label, margin: '0 0 8px' } }, '口径提示：金额按每轮各自的时刻判峰谷。若某会话在插件加载前已有用量，那部分按当前档位线性估算（卡片数字悬停可看覆盖比例），加载后的轮次是精确的；占比一律是估算。'),
      ...
      h('div', { style: { marginTop: '12px', paddingTop: '10px', borderTop: '1px solid ' + HAIRLINE } },
        ...
        sessions !== null && listError === null && sessions.length > 0 ? h('div', {
          ...
          h('span', { style: { whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', color: MUTED } }, s.id.slice(0, 22)),
