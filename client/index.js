...
function Card(props) {
  ...
  return h('div', { style: box },
    ...
    (data.subagents && data.subagents.count > 0) ? h('div', { style: { borderTop: '1px solid ' + HAIRLINE, marginTop: '8px', paddingTop: '7px' } },
      ...
      // 明细：每个子代理一行（独立会话，用量取它自己的四桶）
      (data.subagents.items || []).slice(0, 8).map((item, index) => h('div', {
        key: item.id || String(index),
        style: { display: 'grid', gridTemplateColumns: '1fr auto auto', gap: '2px 8px', marginTop: '4px', fontSize: '11px' },
      },
        h('span', { title: item.note || item.id || '', style: { whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', color: MUTED } },
          '· ' + (item.label || String(item.id || '').slice(0, 12)) + (item.mode === 'live' ? '（进行中）' : '')),
        h('span', { style: { ...muted, textAlign: 'right', minWidth: '46px', ...mono } }, item.tokens == null ? '—' : fmtCount(item.tokens) + ' tok'),
        h('span', { style: { textAlign: 'right', minWidth: '46px', ...mono } }, showAmount ? (item.costCny == null ? '—' : fmtCny(item.costCny)) : ''))),

      (data.subagents.items || []).length > 8
        ? h('div', { style: { ...muted, marginTop: '4px' } }, '…另有 ' + (data.subagents.items.length - 8) + ' 个')
        : null,
